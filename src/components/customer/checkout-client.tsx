...
function validateOrderInput(input: ValidationInput): string | null {
  if (!input.name.trim()) {
  ...
  if (input.paymentMethod === "UPI") {
    if (!input.upiIdOrNumber.trim()) {
      return "Please enter your UPI ID or Mobile number";
    }
  }
  ...
  if (input.paymentMethod === "CARD") {
    ...
    const rawDigits = input.cardNumber.replace(/\s/g, "");
    if (rawDigits.length < 15) {
      return "Please enter a valid 16-digit card number";
    }
    if (input.cardExpiry.length < 5) {
      return "Please enter card expiry date (MM/YY)";
    }
    if (input.cardCvc.length < 3) {
      return "Please enter 3-digit CVV/CVC";
    }
  }
...
function buildStructuredNotes(
  notes: string,
  paymentMethod: PaymentMethod,
  details: { upiIdOrNumber: string; cardHolder: string; cardNumber: string; cardExpiry: string }
): string {
  const baseNotes = notes.trim();
  let paymentNote = "";

  if (paymentMethod === "UPI") {
    paymentNote = `[UPI Payment: ${details.upiIdOrNumber.trim()}]`;
  } else if (paymentMethod === "CARD") {
    const last4 = details.cardNumber.replace(/\s/g, "").slice(-4);
    paymentNote = `[Card Payment: Holder ${details.cardHolder.trim()} (Ending ****${last4}) Exp ${details.cardExpiry}]`;
  } else if (paymentMethod === "CASH") {
    paymentNote = `[Cash Payment: Pay at Table / Counter]`;
  }

  if (!paymentNote) return baseNotes;
  return baseNotes ? `${baseNotes} | ${paymentNote}` : paymentNote;
}
...
function UpiPaymentSection({
  ...
  upiIdOrNumber: string;
  setUpiIdOrNumber: (val: string) => void;
}>) {
  return (
    <div className="pt-2 space-y-4 animate-in fade-in duration-200">
      ...
      <div className="space-y-1.5">
        <label htmlFor="upi-id-input" className="text-xs font-bold text-gray-700 flex items-center gap-1">
...
function CashPaymentSection({ total }: Readonly<{ total: number }>) {
  return (
...
interface CardPaymentSectionProps {
  ...
  cardCvc: string;
  onCardCvcChange: (e: React.ChangeEvent<HTMLInputElement>) => void;
}
...
function CardPaymentSection({
  ...
  cardCvc,
  onCardCvcChange,
}: Readonly<CardPaymentSectionProps>) {
  return (
    <div className="pt-2 space-y-4 animate-in fade-in duration-200">
      ...
      <div className="space-y-3">
        {/* Holder Name */}
        <div>
          <label htmlFor="card-holder-input" className="text-xs font-bold text-gray-700 block mb-1">
        ...
        {/* Card Number */}
        <div>
          <label htmlFor="card-number-input" className="text-xs font-bold text-gray-700 block mb-1">
        ...
        <div className="grid grid-cols-2 gap-3">
          <div>
            <label htmlFor="card-expiry-input" className="text-xs font-bold text-gray-700 block mb-1">
          ...
          <div>
            <label htmlFor="card-cvc-input" className="text-xs font-bold text-gray-700 block mb-1">
...
function PlaceOrderButtonContent({
  isLoading,
  paymentMethod,
}: Readonly<{
  isLoading: boolean;
...
function getButtonColorClass(paymentMethod: PaymentMethod): string {
  if (paymentMethod === "UPI") {
    return "bg-purple-600 hover:bg-purple-700 text-white shadow-purple-600/25";
  }
  if (paymentMethod === "CARD") {
    return "bg-blue-600 hover:bg-blue-700 text-white shadow-blue-600/25";
  }
...
export function CheckoutClient() {
  ...
  const handlePlaceOrder = async () => {
    ...
    const structuredNotes = buildStructuredNotes(notes, paymentMethod, {
      upiIdOrNumber,
      cardHolder,
      cardNumber,
      cardExpiry,
        ...
        <section className="bg-white p-5 sm:p-6 rounded-3xl shadow-xs border border-gray-100 space-y-4">
          ...
          <div className="grid grid-cols-3 gap-2.5">
            ...
            <button
              ...
              className={`p-3.5 rounded-2xl border text-center flex flex-col items-center gap-1.5 transition-all cursor-pointer ${paymentMethod === "UPI"
            ...
            <button
              ...
              className={`p-3.5 rounded-2xl border text-center flex flex-col items-center gap-1.5 transition-all cursor-pointer ${paymentMethod === "CASH"
            ...
            <button
              ...
              className={`p-3.5 rounded-2xl border text-center flex flex-col items-center gap-1.5 transition-all cursor-pointer ${paymentMethod === "CARD"
          ...
          {/* ========================================================================= */}
          {/* TAB CONTENT 1: UPI PAYMENT */}
          {/* ========================================================================= */}
          {paymentMethod === "UPI" && (
            <UpiPaymentSection
              restaurantName={restaurantName}
              upiIdOrNumber={upiIdOrNumber}
              setUpiIdOrNumber={setUpiIdOrNumber}
            />
          )}

          {/* ========================================================================= */}
          {/* TAB CONTENT 2: CASH PAYMENT */}
          {/* ========================================================================= */}
          {paymentMethod === "CASH" && <CashPaymentSection total={total} />}

          {/* ========================================================================= */}
          {/* TAB CONTENT 3: CARD PAYMENT */}
          {/* ========================================================================= */}
          {paymentMethod === "CARD" && (
            <CardPaymentSection
              restaurantName={restaurantName}
      ...
      <footer className="fixed bottom-0 left-0 w-full z-40 bg-white/95 backdrop-blur-md border-t border-gray-100 shadow-[0_-10px_40px_rgba(0,0,0,0.06)] pb-safe">
        <div className="max-w-md mx-auto p-4 flex items-center justify-between gap-4">
          ...
          <button
            ...
            disabled={isLoading}
            className={`flex-1 py-3.5 rounded-2xl font-bold text-sm shadow-md transition-all active:scale-[0.98] disabled:opacity-50 flex items-center justify-center gap-2 ${getButtonColorClass(paymentMethod)}`}
          >
            <PlaceOrderButtonContent isLoading={isLoading} paymentMethod={paymentMethod} />
          </button>
        </div>
      </footer>
    </div>
